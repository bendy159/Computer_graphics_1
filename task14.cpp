...
void draw_triangle(vector<unsigned char>& image, vector<double>& z_buffer, double W, double H, vector<Vertex>& vertices, vector<Face>& faces, double scale, double offset_x, double offset_y) {
...
int main() {
    ...
    draw_triangle(image, z_buffer, W, H, vertices, faces, scale, offset_x, offset_y);

    lodepng::encode("poly_4.png", image, W, H, LCT_RGB, 8);
