...
vector<Vertex> read_obj_vertices(const string filename) {        //Взял из 3 задачи
...
vector<Face> read_obj_faces(const string filename) {     //Взял из 5 задачи
    vector<Face> faces;
    ifstream file(filename);
...
vector<texture_cord> read_obj_texture_coords(const string& filename) {
    vector<texture_cord> texCoords;
    ifstream file(filename);
...
vector<unsigned char> load_texture(const string& filename, unsigned& width, unsigned& height) {
...
void get_texture_color(const vector<unsigned char>& texture, unsigned width, unsigned height, 
                      double u, double v, unsigned char& r, unsigned char& g, unsigned char& b) {

    int tex_x = round(u * (width - 1));
    int tex_y = round((1.0 - v) * (height - 1));
    
    int index = 3 * (tex_y * width + tex_x);
    
    if (index >= 0 && index + 2 < texture.size()) {
        r = texture[index];
        g = texture[index + 1];
        b = texture[index + 2];
    } else {
...
void draw_triangle(vector<unsigned char>& image, vector<double>& z_buffer, double W, double H,
    vector<Vertex>& vertices, vector<Face>& faces, vector<texture_cord>& texCoords, 
    vector<unsigned char>& texture, unsigned texWidth, unsigned texHeight,
    double a_X, double a_Y, double u_0, double v_0) {
    ...
    for(int i = 0; i < faces.size(); ++i) {
        ...
        texture_cord tex0 = texCoords[faces[i].vt1 - 1];
        texture_cord tex1 = texCoords[faces[i].vt2 - 1];
        texture_cord tex2 = texCoords[faces[i].vt3 - 1];
            ...
            for(int y = ymin; y <= ymax; ++y) {
                ...
                if(bar_cords.x >= 0 && bar_cords.y >= 0 && bar_cords.z >= 0) {
                    ...
                    if(z_value < z_buffer[index / 3]) {
                        ...
                        double u = bar_cords.x * tex0.u + bar_cords.y * tex1.u + bar_cords.z * tex2.u;
                        double v = bar_cords.x * tex0.v + bar_cords.y * tex1.v + bar_cords.z * tex2.v;

                        unsigned char r, g, b;
                        get_texture_color(texture, texWidth, texHeight, u, v, r, g, b);
                        double intensity = -(l.x * bar_cords.x + l.y * bar_cords.y + l.z * bar_cords.z);
                        intensity = max(0.3, min(1.0, intensity));

                        image[index] = r * intensity;     // R
                        image[index + 1] = g * intensity; // G  
                        image[index + 2] = b * intensity; // B
...
int main() {
    ...
    cout << '2';
    vector<texture_cord> texCoords = read_obj_texture_coords(filename);
    cout << '3';
    ...
    double beta = 0;   // Поворот вокруг оси Y
    double gamma = 0;  // Поворот вокруг оси Z

    unsigned texWidth, texHeight;
    vector<unsigned char> texture = load_texture("textures/bunny-atlas.png", texWidth, texHeight);
    cout << '4';
    ...
    int z0 = 3;
    vector<Vertex> transformed_vertices = transform_vertices(vertices, alpha, beta, gamma, 0, 0, z0);

    cout << '5';
    draw_triangle(image, z_buffer, W, H, transformed_vertices, faces, texCoords, texture, texWidth, texHeight, a_X, a_Y, u_0, v_0);

    cout << '6';


    string output_name = "poly_17.png";
    lodepng::encode(output_name, image, W, H, LCT_RGB, 8);
    ...
    while(true) {
        ...
        switch(key) {
            case('a'): {beta += 0.1; break;}
            case('d'): {beta -= 0.1; break;}
            case('w'): {alpha += 0.1; break;}
            case('s'): {alpha -= 0.1; break;}
            case('f'): {z0 += 1000; break;}
            case('g'): {z0 -= 1000; break;}
            case('q'): {gamma -= 0.1; break;}
            case('e'): {gamma += 0.1; break;}
        }
