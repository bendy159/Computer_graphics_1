...
void draw_triangle(vector<unsigned char>& image, vector<double>& z_buffer, double W, double H,
    vector<Vertex>& vertices, vector<Face>& faces, double scale, double offset_x, double offset_y) {
...
int main() {
    ...
    double alpha = 3.14;  // Поворот вокруг оси X
    double beta = 3.14;   // Поворот вокруг оси Y
    double gamma = 0.0;  // Поворот вокруг оси Z

    vector<Vertex> transformed_vertices = transform_vertices(vertices, alpha, beta, gamma, 0, 0, 0);

    double min_x = transformed_vertices[0].x;
    double max_x = transformed_vertices[0].x;     //Границы картинки
    double min_y = transformed_vertices[0].y;
    double max_y = transformed_vertices[0].y;

    for(int i = 0; i < (int)transformed_vertices.size(); i++) {         //Ну и теперь ищем минимальную и максимальную границу. Долго но зато красиво
    ...
    double scale_x = (double)(0.8 * W) / (max_x - min_x);  //Вычисляем масштаб так, чтобы отступ по 10% с каждой стороны если в центре
    double scale_y = (double)(0.8 * H) / (max_y - min_y);
    double scale = min(scale_x, scale_y);  // Сохраняем пропорции

    double center_x = (min_x + max_x) / 2.0;
    double center_y = (min_y + max_y) / 2.0;

    double offset_x = -center_x * scale + W / 2.0;  // Центрирование по X
    double offset_y = -center_y * scale + H / 2.0;  // Центрирование по Y

    vector<unsigned char> image(H * W * 3, 0);  //Хачу черный фон
    
    vector<double> z_buffer(H * W, Z_BUF);

    draw_triangle(image, z_buffer, W, H, transformed_vertices, faces, scale, offset_x, offset_y);

    lodepng::encode("poly_6.png", image, W, H, LCT_RGB, 8);
